...
class  Basic_Api extends Base_Controller {
    ...
    public function get_packages_post(){
        ...
        $user_info = $this->Api_Model->get_rows_info(TBL_USER, "id, company_id, active", $where_condition);
        if(isset($user_info['id']) && $user_info['id'] > 0){
    ...
    public function update_user_step1_post(){
        ...
        $user_info = $this->Api_Model->get_rows_info(TBL_USER, "id, referral_id, company_id", $where_condition);
        if(isset($user_info['id']) && $user_info['id'] > 0){
            $company_info = $this->Api_Model->get_rows_info(TBL_COMPANY, "*", array('id' => $user_info['company_id'], 'active' => 1));
            $company_type = $company_info['type'];
            $referral_id = $user_info['referral_id'];
    ...
    public function proceed_update_user_step1_post($user_id, $package_id){
        ...
        if($company_id != 2 && $total_topup < $package_quantity && $is_paid_to_company == 1){
    ...
    public function get_user_information_post(){
        ...
        $user_info = $this->Api_Model->get_rows_info(TBL_USER, "id, is_voucher, voucher_id, fullname, username, phone_no, ic, address_line1, postcode, city, state, package_id, country_id, bank_name, account_name, account_no, is_step1, is_step2, is_step3, is_step4, is_done, referral_id, company_id", $where_condition);
        if(isset($user_info['id']) && $user_info['id'] > 0){
    ...
    public function update_user_final_step_post(){
        ...
        $user_info = $this->Api_Model->get_rows_info(TBL_USER, "id, package_id, company_id, pincode, voucher_id, referral_id", $where_condition);
        if(isset($user_info['id']) && $user_info['id'] > 0){
