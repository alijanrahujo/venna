...
class Order extends Base_Controller {
    ...
    public function checking(){
        $this->load(ADMIN_URL . "/shipment_checking_list");
    }

    public function shipping(){
        $this->load(ADMIN_URL . "/shipping_fee_list");
    }

    // public function update_old_member_amount_to_zero(){
    //     $agent_list = $this->Api_Model->get_rows(TBL_USER, "id", array('is_old' => 1, 'active' => 1));
    //     foreach($agent_list as $row_agent){
    //         $purchase_package_info = $this->Api_Model->get_rows_info(TBL_PURCHASE_PACKAGE, "id", array('user_id' => $row_agent['id'], 'active' => 1, 'company_id' => 1), "id", "ASC", 1);
    //         $purchase_id = isset($purchase_package_info['id']) ? $purchase_package_info['id'] : 0;

    //         if($purchase_id != 0){
    //             $this->Api_Model->update_data(TBL_PURCHASE_PACKAGE, array('user_id' => $row_agent['id'], 'active' => 1, 'id' => $purchase_id, 'company_id' => 1), array('amount' => 0));
    //         }
    //     }
    ...
    // public function update_cb_point_by_brand(){
    //     $cb_point_list = $this->Api_Model->get_rows(TBL_CB_POINT, "*", array('active' => 1, 'company_id' => 7));
    //     // debugPrintArr($cb_point_list); die;
    //     foreach($cb_point_list as $row_cb_point){
    //         $user_id = $row_cb_point['user_id'];

    //         $current_credit = $row_cb_point['credit'];

    //         if($row_cb_point['description'] != "Transfer From Old System"){
    //             $new_credit = $current_credit * 10;
    //         }else{
    //             $new_credit = $current_credit;
    //         }
    //         $total_balance = $this->check_cb_point_update_balance_post($user_id, $row_cb_point['id']);
    //         $new_balance = $total_balance + $new_credit;
    //         $this->Api_Model->update_data(TBL_CB_POINT, array('id' => $row_cb_point['id'], 'active' => 1, 'company_id' => 7, 'user_id' => $user_id, 'description !=' => "Transfer From Old System"), array('credit' => $new_credit));
    //         $this->Api_Model->update_data(TBL_CB_POINT, array('id' => $row_cb_point['id'], 'active' => 1, 'company_id' => 7, 'user_id' => $user_id), array('balance' => $new_balance));
    //     }

    //     $this->load->view("output/success_response");
    // }
    
    public function epackage(){
        $company_id = $this->user_profile_info['company_id'];
        $id = $this->encryption->decrypt(str_replace(array('-', '_', '~'), array('+', '/', '='), $this->uri->segment(3)));
        $edit_info = $this->Api_Model->get_rows_info(TBL_PURCHASE_PACKAGE, "*", array('id' => $id, 'active' => 1));
        $this->page_data['package'] = $this->Api_Model->get_rows(TBL_PACKAGE, "*", array('active' => 1, 'company_id' => $company_id));
        $this->page_data['edit'] = $edit_info;
        $this->load(ADMIN_URL . "/edit_purchase_package", $this->page_data);
    }
    
    public function eorder(){
        $id = $this->encryption->decrypt(str_replace(array('-', '_', '~'), array('+', '/', '='), $this->uri->segment(3)));
        $edit_info = $this->Api_Model->get_rows_info(TBL_ORDER, "*", array('id' => $id, 'active' => 1));
        $this->page_data['edit'] = $edit_info;
        $this->load(ADMIN_URL . "/edit_shipment_order", $this->page_data);
    }

    public function update_purchase_package(){
        $purchase_package_id = isset($this->request_data['purchase_package_id']) ? $this->request_data['purchase_package_id'] : 0;
        ...
        $subtotal = isset($this->request_data['subtotal']) ? $this->request_data['subtotal'] : 0;
        $pv = isset($this->request_data['pv']) ? $this->request_data['pv'] : 0;

        $purchase_package_info = $this->Api_Model->get_rows_info(TBL_PURCHASE_PACKAGE, "*", array('id' => $purchase_package_id, 'active' => 1));
        if(isset($purchase_package_info['id']) && $purchase_package_info['id'] > 0){
            $current_package_id = $purchase_package_info['package_id'];
            $current_is_voucher = $purchase_package_info['is_voucher'];
            $is_restock = $purchase_package_info['is_restock'];
            $current_restock_quantity = $purchase_package_info['quantity'];
            if($current_is_voucher == 1){
                $user_info = $this->Api_Model->get_rows_info(TBL_USER, "id, package_id, voucher_id", array('id' => $purchase_package_info['user_id'], 'active' => 1));
                $user_voucher_id = isset($user_info['id']) ? $user_info['voucher_id'] : 0;
                $current_package_info = $this->Api_Model->get_rows_info(TBL_BIG_PRESENT, "*", array('id' => $user_voucher_id, 'active' => 1));
                $current_package_amount = isset($current_package_info['id']) ? $current_package_info['total_stock'] : 0;
            }else{
                $current_package_info = $this->Api_Model->get_rows_info(TBL_PACKAGE, "*", array('id' => $current_package_id, 'active' => 1));
                $current_package_amount = isset($current_package_info['id']) ? $current_package_info['quantity'] : 0;
                $user_voucher_id = 0;
            }

            $company_info = $this->Api_Model->get_rows_info(TBL_COMPANY, "id, type", array('id' => $purchase_package_info['company_id'], 'active' => 1));
            ...
            if($company_type == "FIXED"){
                ...
                $package_amount = isset($package_info['id']) ? $package_info['quantity'] : 0;
                $package_unit_price = isset($package_info['id']) ? $package_info['unit_price'] : 0;
                $stock_balance = $this->check_stock_balance_post($purchase_package_info['user_id']);

                $package_amount = $quantity;
                $is_paid_to_company = $is_company;
                ...
                if($is_restock == 1){
                    $package_amount = $current_restock_quantity;
                    $package_subtotal = $package_unit_price * $package_amount;
                    $data_purchase_package['amount'] = $package_amount;
                    $data_purchase_package['subtotal'] = $package_subtotal;
                }
                ...
                if($voucher_code != ""){
                    $voucher_info = $this->Api_Model->get_rows_info(TBL_BIG_PRESENT, "*", array('code' => $voucher_code, 'active' => 1, 'balance_quantity !=' => 0));

                    if(isset($voucher_info['id']) && $voucher_info['id'] > 0){
                        $current_voucher_quantity = $voucher_info['quantity'];
                        $new_voucher_quantity = $current_voucher_quantity - 1;
                        $data_purchase_package['is_voucher'] = 1;
                        $data_user['is_voucher'] = 1;
                        $data_user['voucher_id'] = $voucher_info['id'];

                        $package_info = $this->Api_Model->get_rows_info(TBL_BIG_PRESENT, "*", array('id' => $voucher_info['id']));
                        $is_paid_to_company = isset($package_info['id']) ? $package_info['is_company'] : 0;
                        $package_amount = isset($package_info['id']) ? $package_info['total_stock'] : 0;
                        $stock_balance = $this->check_stock_balance_post($purchase_package_info['user_id']);

                        if($purchase_package_info['status'] == "APPROVE"){
                            $new_stock_balance = $stock_balance - $current_package_amount;
                            $latest_stock_balance = $new_stock_balance + $package_amount;
                            $this->proceed_stock_post($purchase_package_info['user_id'], $purchase_package_info['company_id'], $purchase_package_info['id'], $current_package_amount, "debit");
                            $this->proceed_stock_post($purchase_package_info['user_id'], $purchase_package_info['company_id'], $purchase_package_info['id'], $package_amount, "credit");
        
                            $data_user['total_stock'] = $latest_stock_balance;
                        }
        
                        $this->Api_Model->update_data(TBL_PURCHASE_PACKAGE, array('id' => $purchase_package_id, 'active' => 1), $data_purchase_package);
                        $this->Api_Model->update_data(TBL_USER, array('id' => $purchase_package_info['user_id'], 'active' => 1), $data_user);
                        $this->Api_Model->update_data(TBL_BIG_PRESENT, array('id' => $voucher_info['id'], 'active' => 1), array('balance_quantity' => $new_voucher_quantity));
                    ...
                    if($user_voucher_id != 0){
                        $voucher_info = $this->Api_Model->get_rows_info(TBL_BIG_PRESENT, "*", array('id' => $user_voucher_id, 'active' => 1));
                        $current_voucher_quantity = $voucher_info['balance_quantity'];
                        $new_voucher_quantity = $current_voucher_quantity + 1;
                        $this->Api_Model->update_data(TBL_BIG_PRESENT, array('id' => $user_voucher_id, 'active' => 1), array('balance_quantity' => $new_voucher_quantity));
                    }
    ...
    public function get_package_order(){
        ...
        $is_agent = isset($this->request_data['is_agent']) ? $this->request_data['is_agent'] : 0;
        ...
        if($user_type == "ADMIN" && $group_id == 1){
            if($is_agent == 1){
                $where_query = array('active' => 1, 'user_id' => $user_id);
            }else{
            ...
            if($is_agent == 1){
                $where_query = array('active' => 1, 'company_id' => $company_id, 'user_id' => $user_id);
            }else{
        ...
        $output_data = array();
        $result['data'] = [];
        $package_order_list = $this->Api_Model->get_datatables_list(TBL_PURCHASE_PACKAGE, "*", $where_query, $where_group_like_query, $where_group_or_like_query, $order_query, $start, $count);
        $counting = 0;

        foreach ($package_order_list as $row) {
            // check the permission for approve btn display
            $admin_info = $this->Api_Model->get_rows_info(TBL_USER, "id, is_approve_package", array('id' => $admin_id));
            $is_approve_package_permission = isset($admin_info['id']) ? $admin_info['is_approve_package'] : 0;
            ...
            $is_voucher = isset($user_info['id']) ? $user_info['is_voucher'] : 0;
            $voucher_id = isset($user_info['id']) ? $user_info['voucher_id'] : 0;
            $package_id = isset($row['id']) ? $row['package_id'] : 0;
            ...
            $email = isset($user_info['id']) ? $user_info['email'] : "";
            $phone_no = isset($user_info['id']) ? $user_info['phone_no'] : "";
            ...
            if($restock_quantity != 0){
                $package_quantity = $restock_quantity;
                $package_total = $package_quantity * $package_price;
                $package_total = number_format($package_total, 2);
            }

            if($restock_subtotal != "0.00"){
                $package_total = $restock_subtotal;
            }

            $row['referral_name'] = $fullname . " (" . $username . ")";
            $row['agent_detail'] = $fullname . " (" . $username . ")" . "<br>" . $email . "<br>" . $phone_no;
            $row['package_detail'] = "#000" . $row['id'] . "<br>" . $package_name . "<br>" . $package_quantity . " x " . $package_price . " = " . $package_total;
            ...
            if($order_status == "PENDING"){
                ...
                if($is_approve_package_permission == 1){
                    $btn .= "<a href='#' class='btn-sm btn-success' style='border:none;' onclick='approve_order(" . $row['id'] . ")'>" . "Approve" . "</a> ";
                }
                $btn .= "<a href='" . site_url() . "Order/epackage/" . $id . "' class='btn-sm btn-info' style='border:none;'>" . "Edit" . "</a> <a href='#' class='btn-sm btn-danger' style='border:none;' onclick='cancel_order(" . $row['id'] . ")'>" . "Cancel" . "</a>";
            }else if($order_status == "APPROVE"){
                $row['order_status'] = '<span class="badge bg-light-success mb-1 mr-2">' . $order_status . '</span>';
                $btn .= "<a href='#' class='btn-sm btn-danger' style='border:none;' onclick='cancel_order(" . $row['id'] . ")'>" . "Cancel" . "</a> ";
                $btn .= "<a href='#' class='btn-sm btn-dark' style='border:none;' onclick='refund_package_order(" . $row['id'] . ")'>" . "Refund Stock" . "</a> ";
                if($row['company_id'] != 8){
                    $btn .= "<a target='_blank' href='" . site_url() . "Order/pgenerate/" . $row['id'] . "' class='btn-sm btn-success' style='border:none;'>" . "Invoice" . "</a> ";
                }
    ...
    public function refund_package_order(){
        $package_id = isset($this->request_data['package_id']) ? $this->request_data['package_id'] : 0;

        $package_info = $this->Api_Model->get_rows_info(TBL_PURCHASE_PACKAGE, "*", array('id' => $package_id, 'active' => 1));
        if(isset($package_info['id']) && $package_info['id'] > 0){
            $package_id = $package_info['id'];
            $total_quantity = $package_info['amount'];
            $user_id = $package_info['user_id'];
            $referral_id = $package_info['referral_id'];
            $company_id = $package_info['company_id'];
            $order_package_id = $package_info['package_id'];
            $package_pv = $package_info['pv'];

            if($referral_id == 0){
                $add_user_id = $user_id;
            }else{
                $add_user_id = $referral_id;
            }
            ...
            if($company_type == "FIXED"){
                $stock_balance = $this->check_stock_balance_post($add_user_id);
                ...
                $data_stock = array(
                    ...
                    'package_id' => $package_id,
                    'description' => "Refund Stock of Package ID #000" . $package_id,
                ...
                $this->Api_Model->update_data(TBL_USER, array('id' => $add_user_id, 'active' => 1), $data_user_update);
            }else{
                $package_info = $this->Api_Model->get_rows_info(TBL_PACKAGE, "id, grand_total", array('id' => $order_package_id, 'active' => 1));
                $package_price = isset($package_info['id']) ? $package_info['grand_total'] : "0.00";

                $point_balance = $this->check_point_balance_post($add_user_id);
                $new_point_balance = $point_balance + $package_price;

                $data_point = array(
                    'user_id' => $add_user_id,
                    'company_id' => $company_id,
                    'package_id' => $package_id,
                    'description' => "Refund Point of Package ID #000" . $package_id,
                    'credit' => $package_price,
                    'balance' => $new_point_balance
                );
                $this->Api_Model->insert_data(TBL_POINT, $data_point);

                $data_point_update = array(
                    'total_point' => $new_point_balance
                );
                $this->Api_Model->update_data(TBL_USER, array('id' => $add_user_id, 'active' => 1), $data_point_update);

                $pv_balance = $this->check_pv_balance_post($add_user_id);
                $new_pv_balance = $pv_balance + $package_pv;

                $data_pv = array(
                    'user_id' => $add_user_id,
                    'company_id' => $company_id,
                    'package_id' => $package_id,
                    'description' => "Refund PV of Package ID #000" . $package_id,
                    'credit' => $package_pv,
                    'balance' => $new_pv_balance
                );
                $this->Api_Model->insert_data(TBL_PV, $data_pv);

                $data_pv_update = array(
                    'total_pv' => $new_pv_balance
                );
                $this->Api_Model->update_data(TBL_USER, array('id' => $add_user_id, 'active' => 1), $data_pv_update);
            }
            
            $data_update = array(
                'status' => "CANCEL"
            );
            $this->Api_Model->update_data(TBL_PURCHASE_PACKAGE, array('id' => $package_id, 'active' => 1), $data_update);
    ...
    public function get_stock_record(){
        $draw = isset($this->request_data['draw']) ? $this->request_data['draw'] : 0;
        $start = isset($this->request_data['start']) ? $this->request_data['start'] : 0;
        $count = isset($this->request_data['length']) ? $this->request_data['length'] : 10;
        $order = isset($this->request_data['order']) ? $this->request_data['order'] : [];
        ...
        $output_data = array();
        $result['data'] = [];
        $stock_record_list = $this->Api_Model->get_datatables_list(TBL_STOCK, "*", $where_query, $where_group_like_query, $where_group_or_like_query, $order_query, $start, $count);
        $counting = 0;

        foreach ($stock_record_list as $row) {
            $package_id = $row['package_id'];
            $order_id = $row['order_id'];

            if($order_id == 0){
                $purchase_package_info = $this->Api_Model->get_rows_info(TBL_PURCHASE_PACKAGE, "id, status, user_id", array('id' => $package_id, 'active' => 1));
                $row['order_id'] = isset($purchase_package_info['id']) ? "#000" . $purchase_package_info['id'] : 0;
                $row['order_status'] = isset($purchase_package_info['id']) ? $purchase_package_info['status'] : "";
            }else{
                $order_info = $this->Api_Model->get_rows_info(TBL_ORDER, "id, status, user_id, referral_id", array('id' => $order_id, 'active' => 1));
                $row['order_id'] = isset($order_info['id']) ? "#000" . $order_info['id'] : 0;
                $row['order_status'] = isset($order_info['id']) ? $order_info['status'] : "";
            }
    ...
    public function cancel_package_order(){
    ...
    public function view_order_receipt(){
        ...
        $order_info = $this->Api_Model->get_rows_info(TBL_ORDER, "id, payment_receipt", array('id' => $id, 'active' => 1));
    ...
    public function approve_package_order(){
        ...
        if(isset($package_order_info['id']) && $package_order_info['id'] > 0){
            ...
            if($is_voucher == 1){
                $user_info = $this->Api_Model->get_rows_info(TBL_USER, "id, voucher_id, country_id, is_old", array('id' => $user_id, 'active' => 1));
                $voucher_id = isset($user_info['id']) ? $user_info['voucher_id'] : 0;
            ...
            if($is_paid_to_company == 0 && $is_continue_giving_voucher){
                ...
                // if(!empty($free_voucher_list) && $is_restock == 0 && $is_old == 0){
            ...
            // var_dump($free_package_list); echo "-" . $is_used_promotion; die;
            ...
            if($is_free_voucher == 1){
                ...
                if(isset($big_present_log_info['id']) && $big_present_log_info['id'] > 0){
                    ...
                    $package_pv = $big_present_info['total_point'];
            ...
            if($company_type == "FIXED" && $is_voucher == 0 && $total_quantity != 60 && $total_quantity != 4){
                if($available_stock_balance == $total_quantity){
                    $is_continue_approved = true;
                }else{
                    if($available_stock_balance < $total_quantity && $is_paid_to_company == 0){
            ...
            $first_acc_info = $this->Api_Model->get_rows_info(TBL_USER, "id", array('company_id' => $company_id, 'user_type' => "AGENT", 'active' => 1, 'is_done' => 1), "id", "ASC", 1);
            $first_brand_acc_id = isset($first_acc_info['id']) ? $first_acc_info['id'] : 0;
            if($first_brand_acc_id == $user_id){
                $is_continue_approved = true;
            }
            ...
            if($is_continue_approved){
                ...
                $this->Api_Model->update_data(TBL_PURCHASE_PACKAGE, array('id' => $id, 'active' => 1), array('invoice_name' => "invoice" . $id . ".pdf"));
                ...
                $upgrade_package_info = $this->Api_Model->get_rows_info(TBL_PACKAGE, "id, quantity, unit_price", array('id' => $package_id, 'active' => 1));
                $upgrade_package_quantity = isset($upgrade_package_info['id']) ? $upgrade_package_info['quantity'] : 0;
                $upgrade_package_price = isset($upgrade_package_info['id']) ? $upgrade_package_info['unit_price'] : 0;
                $upgrade_package_grand_total = $restock_subtotal;

                $current_package_info = $this->Api_Model->get_rows_info(TBL_PACKAGE, "id, quantity, unit_price, grand_total", array('id' => $current_package_id, 'active' => 1));
                $current_package_quantity = isset($current_package_info['id']) ? $current_package_info['quantity'] : 0;
                $current_package_price = isset($current_package_info['id']) ? $current_package_info['unit_price'] : 0;
                ...
                if($company_type == "FIXED"){
                    ...
                    if($upgrade_package_grand_total > $current_package_grand_total && $upgrade_package_grand_total != 0 && $current_package_grand_total != 0){
                        $upgrade_package_info = $this->Api_Model->get_rows_info(TBL_UPGRADE, "*", array('user_id' => $user_id, 'status' => "PENDING", 'active' => 1));
                        $data_upgrade = array('status' => "APPROVE");
                        $this->Api_Model->update_data(TBL_UPGRADE, array('user_id' => $user_id, 'status' => "PENDING", 'active' => 1), $data_upgrade);
                        $data_user_upgrade = array('package_id' => $package_id);
    ...
    public function approve_product_order(){
        ...
        if(isset($order_info['id']) && $order_info['id'] > 0){
            ...
            $this->Api_Model->update_multiple_data(TBL_ORDER_DETAIL, array('order_id' => $id), $data_update_order_detail);
            ...
            if($company_type == "FIXED"){
                ...
                if($order_type == "normal" || $order_type == "restock" || $order_type == "rb" || $order_type == "drb"){
                    if($order_type != "rb" && $order_type != "drb" && $order_type != "mms"){
                ...
                if($order_type == "normal"){
                    ...
                    $total_point_balance = $this->check_point_balance_post($referral_id);
                ...
                $new_point_balance = $total_point_balance - $total_price;
    ...
    public function cancel_retail_order(){
        $order_id = isset($this->request_data['order_id']) ? $this->request_data['order_id'] : 0;
    ...
    public function refund_stock_order(){
        $order_id = isset($this->request_data['order_id']) ? $this->request_data['order_id'] : 0;
        ...
        if(isset($order_info['id']) && $order_info['id'] > 0){
            ...
            if($company_type == "FIXED"){
                ...
                $data_stock = array(
                    ...
                    'company_id' => $company_id,
                    'order_id' => $order_id,
                    'description' => "Refund Stock of Order ID #000" . $order_id,
                    'credit' => $total_quantity,
                    'balance' => $new_balance
                );
                $this->Api_Model->insert_data(TBL_STOCK, $data_stock);
                ...
                $point_balance = $this->check_point_balance_post($add_user_id);
                $new_point_balance = $point_balance + $total_price;

                $data_point = array(
                    'user_id' => $add_user_id,
                    'company_id' => $company_id,
                    'order_id' => $order_id,
                    'description' => "Refund Point of Order ID #000" . $order_id,
                    'credit' => $total_price,
                    'balance' => $new_point_balance
                );
                $this->Api_Model->insert_data(TBL_POINT, $data_point);
                ...
                $order_detail_info = $this->Api_Model->get_rows_info(TBL_ORDER_DETAIL, "id, SUM(pv_subtotal) as total_pv_subtotal", array('order_id' => $order_id, 'active' => 1));
                $total_pv_subtotal = isset($order_detail_info['id']) ? $order_detail_info['total_pv_subtotal'] : 0;
                $pv_balance = $this->check_pv_balance_post($add_user_id);
                $new_pv_balance = $pv_balance + $total_pv_subtotal;

                $data_pv = array(
                    'user_id' => $add_user_id,
                    'company_id' => $company_id,
                    'order_id' => $order_id,
                    'description' => "Refund PV of Order ID #000" . $order_id,
                    'credit' => $total_pv_subtotal,
                    'balance' => $new_pv_balance
                );
                $this->Api_Model->insert_data(TBL_PV, $data_pv);
    ...
    public function revert_cancel_order(){
        $order_id = isset($this->request_data['order_id']) ? $this->request_data['order_id'] : 0;
        ...
        if(isset($order_info['id']) && $order_info['id'] > 0){
            ...
            $data_stock = array(
                ...
                'company_id' => $company_id,
                'order_id' => $order_id,
                'description' => "Revert Stock of Order ID #000" . $order_id,
                'debit' => $total_quantity,
                'balance' => $new_balance
            );
            $this->Api_Model->insert_data(TBL_STOCK, $data_stock);
    ...
    public function check_break_away_requirement($type = "", $order_id, $user_id, $is_personal_group_sales = false){
        ...
        if(($break_away_conditions || $two_max_package_conditions || $personal_group_total_conditions['data']['status']) && $type != ""){
            ...
            if($break_away_conditions){
                ...
                $order_purchase_package_info = $this->Api_Model->get_rows_info(TBL_PURCHASE_PACKAGE, "id, package_id", array('id' => $purchase_id));
                $order_user_package_id = isset($order_purchase_package_info['id']) ? $order_purchase_package_info['package_id'] : 0;
                $package_info = $this->Api_Model->get_rows_info(TBL_PACKAGE, "id, unit", array('id' => $order_user_package_id, 'active' => 1));
                $package_unit = isset($package_info['id']) ? $package_info['unit'] : "";
    ...
    public function check_break_away_team_total_box($insert_id = 0, $month, $year, $is_smart_partner_check = false, $debug = false){
        ...
        if(isset($member_info['id']) && $member_info['id'] > 0){
            ...
            if($company_type == "FIXED"){
                ...
                $referral_package_id = isset($referral_info['id']) ? $referral_info['package_id'] : 0;
                $referral_company_id = isset($referral_info['id']) ? $referral_info['company_id'] : 0;
                $referral_stock = $this->get_balance_purchase_quantity_total($month, $referral_id);
                ...
                $referral_total_stock = $this->get_balance_purchase_quantity_total($month, $referral_id, 1);
    ...
    public function insert_smart_partner_total_sales($month, $company_id){
        ...
        if(!empty($smart_partner_arr)){
            foreach($smart_partner_arr as $smart_partner_id){
                ...
                $company_sales = $this->check_package_total_post("", $month, $company_id);
                $total_quantity = $this->check_package_total_post($smart_partner_id, $month, $company_id);
                ...
                if($company_sales != 0 && $total_quantity != 0){
                    ...
                    // if($smart_partner_id == 7){
                        if($is_got_upline){
                            ...
                            if($break_away_conditions || $two_max_package_conditions || $personal_group_total_conditions['data']['status']){
                                ...
                                $company_info = $this->Api_Model->get_rows_info(TBL_COMPANY, "*", array('id' => $company_id, 'active' => 1));
                                $is_smart_partner_half = isset($company_info['id']) ? $company_info['is_smart_partner_half'] : 0;
                                ...
                                if(!$break_away_conditions && $two_max_package_conditions){
                                    ...
                                    if($is_smart_partner_half == 1){
                                        ...
                                        $grand_total_sales_before_pass = $total_sales;
                                        $self_total_sales_pass_to_upline = $total_sales / 2;
                                        $self_total_sales_pass_to_upline1 = $self_total_sales_pass_to_upline / 2;
                                        $self_total_sales_pass_to_upline2 = $self_total_sales_pass_to_upline / 2;
                                    }
    ...
    public function get_mdb_group_quantity($insert_id = 0, $month){
        ...
        if(isset($member_info['id']) && $member_info['id'] > 0){
            ...
            while(count($downline_list) > 0){
                ...
                if(count($personal_group_list) > 0){
                    foreach($personal_group_list as $row_downline){
                        ...
                        $member_id = isset($member['id']) ? $member['id'] : 0;
                        $member_package_id = isset($member['id']) ? $member['package_id'] : 0;
                        $member_company_id = isset($member['id']) ? $member['company_id'] : 0;
                        // $total_stock = $this->get_purchase_quantity_total($month, $member_id, $member_package_id, $member_company_id);
                        $total_stock = $this->get_purchase_quantity_total($month, $member_info['id'], $member_info['package_id'], $member_info['company_id']);
                        if($insert_id != $member_info['id']){
                            $total_quantity += $total_stock;
                        }
                        // if($insert_id == 618){
                        //     debugPrintArr($total_quantity);
                        // }

                        // if($total_quantity != 0 && $insert_id == 111){
                        //     echo $total_stock . "-" . $member_id;
                        // }
                    }
                }
                else{
        ...
        // if($insert_id == 618){
        //     echo "618 : " . $total_quantity;
        // }

        // if($insert_id == 164){
        //     echo "164 : " . $total_quantity;
        // }
    ...
    public function calculate_monthly_bonus(){
        ...
        if(!empty($mdb_qualified_member)){
            if(count($mdb_qualified_list) > 0){
                ...
                // $data['message'] = "Feature is under maintaince now !";
                // $this->load->view("output/error_response", $data);

                $qualified_member_arr = array();
                foreach($mdb_qualified_member as $mdb_member_id){
                    ...
                    $referral_info = $this->Api_Model->get_rows_info(TBL_USER, "id, referral_id", array('id' => $mdb_member_id, 'active' => 1));
                    $referral_id = isset($referral_info['id']) ? $referral_info['referral_id'] : 0;

                    $member_group_sales_info = $this->get_purchase_group_sales_mdb_total($month, $mdb_member_id, $company_id, $referral_id);
                    $group_sales = $member_group_sales_info['data']['total_box'];
                    $purchase_referral_id = $member_group_sales_info['data']['referral_id'];
                    $referral_box = $member_group_sales_info['data']['total_referral_box'];

                    if($group_sales >= $min_mdb_quantity && $group_sales != 0){
                        $self_box = $group_sales - $referral_box;

                        $monthly_bonus_info = $this->Api_Model->get_info_sql(TBL_MONTHLY_BONUS, "id, bonus", "WHERE active = '1' AND company_id = '$company_id' AND (from_amount <= '$group_sales' AND to_amount >= '$group_sales')");
                        $bonus_per_box = isset($monthly_bonus_info['id']) ? $monthly_bonus_info['bonus'] : "0.00";
                        $self_bonus = $bonus_per_box * $self_box;
                        $referral_bonus = $bonus_per_box * $referral_box;
                        ...
                        $data_self = array(
                            ...
                            'total_quantity' => $self_box,
                            'group_sales' => $group_sales,
                            'bonus_per_box' => $bonus_per_box,
                            'bonus' => $self_bonus
                        );
                    
                        if($self_box != 0 && $bonus_per_box != "0.00"){
                            $this->Api_Model->insert_data(TBL_MONTHLY_BONUS_REPORT, $data_self);

                            $mdbm_remark = "MDS (" . strtoupper($month) . ") | " . $self_box . " Qty @ " . $bonus_per_box . " per Qty";
                            $this->give_cash_wallet_comm($mdb_member_id, 0, "mdbm", $mdbm_remark, $self_bonus);
                        }
                        ...
                        $data_referral = array(
                            ...
                            'year' => date("Y"),
                            'company_id' => $company_id,
                            'user_id' => $purchase_referral_id,
                            'total_quantity' => $referral_box,
                            'group_sales' => $group_sales,
                            'bonus_per_box' => $bonus_per_box,
                            'bonus' => $referral_bonus
                        );
                    
                        if($referral_box != 0 && $bonus_per_box != "0.00"){
                            $this->Api_Model->insert_data(TBL_MONTHLY_BONUS_REPORT, $data_referral);

                            $mdbm_remark = "MDS (" . strtoupper($month) . ") | " . $referral_box . " Qty @ " . $bonus_per_box . " per Qty";
                            $this->give_cash_wallet_comm($purchase_referral_id, 0, "mdbm", $mdbm_remark, $referral_bonus);
                        }
                    }
    ...
    public function calculate_mms_bonus(){
        ...
        if(count($mms_qualified_list) > 0){
            ...
            foreach($member_list as $row_member){
                $mms_order_info = $this->Api_Model->get_rows_info(TBL_ORDER, "id, SUM(total_quantity) as total_mms_quantity", array('user_id' => $row_member['id'], 'active' => 1, 'type' => "mms", 'MONTH(insert_time)' => $month, 'YEAR(insert_time)' => $year, 'status' => "APPROVE", 'order_status' => "SHIPPED"));
    ...
    public function get_purchase_quantity_total($month, $user_id, $package_id, $company_id){
        ...
        $purchase_package_info = $this->Api_Model->get_rows_info(TBL_PURCHASE_PACKAGE, "id, SUM(amount) as total_quantity", array('user_id' => $user_id, 'package_id' => $package_id, 'company_id' => $company_id, 'MONTH(insert_time)' => $month, 'active' => 1, 'status' => "APPROVE", 'amount !=' => 0));
    ...
    public function get_purchase_group_sales_mdb_total($month, $user_id, $company_id, $referral_id){
        $result['data'] = [];
        ...
        $total_referral_box = 0;
        $purchase_id = implode("','", $purchase_package_self_arr);
        // if($user_id == 164){
        //     $referral_purchase_package_info = $this->Api_Model->get_info_sql(TBL_PURCHASE_PACKAGE, 'id, SUM(amount) as total_quantity', "WHERE active = '1' AND user_id = '$referral_id' AND company_id = '$company_id' AND MONTH(insert_time) = '$month' AND status = 'APPROVE' AND amount != 0 AND id NOT IN ('" . $purchase_id . "')", true);
        // }else{
        ...
        // }
        // $referral_purchase_package_info = $this->Api_Model->get_rows_info(TBL_PURCHASE_PACKAGE, "id, SUM(amount) as total_quantity", array('referral_id' => $referral_id, 'company_id' => $company_id, 'MONTH(insert_time)' => $month, 'active' => 1, 'status' => "APPROVE", 'amount !=' => 0, 'user_id !=' => $user_id));
        ...
        if($total_stock >= $min_mdb_quantity){
            ...
            $total_referral_box += $referral_total_stock;
        }

        $row['referral_id'] = $referral_id;
        $row['total_box'] = $total_box;
        $row['total_referral_box'] = $total_referral_box;
        $result['data'] = $row;
    ...
    public function check_package_total_post($user_id = 0, $month, $company_id = 0){
        if($user_id == 0){
            $package_info = $this->Api_Model->get_rows_info(TBL_PURCHASE_PACKAGE, 'id, SUM(amount) as total_quantity', array('active' => 1, 'is_company' => 1, 'MONTH(insert_time)' => $month, 'YEAR(insert_time)' => date("Y"), 'amount !=' => 0, 'status' => "APPROVE", 'company_id' => $company_id));
            // $package_info = $this->Api_Model->get_rows_info(TBL_PURCHASE_PACKAGE, 'id, SUM(amount) as total_quantity', array('active' => 1, 'is_company' => 1, 'MONTH(insert_time)' => $month, 'YEAR(insert_time)' => date("Y"), 'amount !=' => 0, 'status' => "APPROVE", 'company_id' => $company_id), "", "", "", array(), array(), array(), true);
        }else{
            $package_info = $this->Api_Model->get_rows_info(TBL_PURCHASE_PACKAGE, 'id, SUM(amount) as total_quantity', array('active' => 1, 'user_id' => $user_id, 'is_company' => 1, 'MONTH(insert_time)' => $month, 'YEAR(insert_time)' => date("Y"), 'amount !=' => 0, 'status' => "APPROVE", 'company_id' => $company_id));
        }
    ...
    public function check_cb_point_update_balance_post($user_id, $id){
        $point_balance = $this->Api_Model->get_rows_info(TBL_CB_POINT, 'SUM(credit) as total_credit, SUM(debit) as total_debit', array('active' => 1, 'user_id' => $user_id, 'id <' => $id));
        $total_credit = isset($point_balance['total_credit']) ? $point_balance['total_credit'] : 0;
        $total_debit = isset($point_balance['total_debit']) ? $point_balance['total_debit'] : 0;
        $total_balance = $total_credit - $total_debit;
        return $total_balance;
    ...
    public function proceed_stock_post($user_id, $company_id, $purchase_id, $total_quantity, $type, $debug = false){
        $purchase_package_info = $this->Api_Model->get_rows_info(TBL_PURCHASE_PACKAGE, "id, is_company", array('id' => $purchase_id, 'active' => 1));
        $is_paid_to_company = isset($purchase_package_info['id']) ? $purchase_package_info['is_company'] : 0;
        $topup_balance = $this->check_topup_balance_post($company_id);
        ...
        if($is_paid_to_company == 1 && $company_id != 2 && $company_id != 11){
    ...
    public function get_order(){
        ...
        $company_id = isset($this->request_data['company_id']) ? $this->request_data['company_id'] : 0;
        $group_id = isset($this->request_data['group_id']) ? $this->request_data['group_id'] : 0;
        $is_check_shipping = isset($this->request_data['is_check_shipping']) ? $this->request_data['is_check_shipping'] : 0;

        $result = array();
        $result['draw'] = $draw;

        if($user_type == "ADMIN" && $group_id == 1){
            if($is_check_shipping == 1){
                $where_query = array('active' => 1, 'delivery_fee !=' => 0.00);
            }else{
                $where_query = array('active' => 1);
            }
            if($status != ""){
            ...
            if($is_check_shipping == 1){
                $where_query = array('active' => 1, 'company_id' => $company_id, 'delivery_fee !=' => 0.00);
            }else{
                $where_query = array('active' => 1, 'company_id' => $company_id);
            }
            if($status != ""){
        ...
        if ($search != "") {
            if($user_type == "ADMIN" && $group_id == 1){
                $where_query = array('o.active' => 1);
                if($status != ""){
                    $where_query['o.status'] = $status;
                }
            }else if($user_type == "ADMIN" && $group_id != 1){
                $where_query = array('o.active' => 1, 'o.company_id' => $company_id);
                if($status != ""){
                    $where_query['o.status'] = $status;
                }
            }
            $where_group_like_query = array('o.id' => $search);
            $left_join_query = "u.id = o.referral_id";
            $where_group_or_like_query = array('u.fullname' => $search, 'u.username' => $search);
            $result['recordsFiltered'] = $this->Api_Model->count_datatables_list(TBL_ORDER . " o", $where_query, $where_group_like_query, $where_group_or_like_query, array(), TBL_USER . " u", $left_join_query);
        } else {
            $result['recordsFiltered'] = $result['recordsTotal'];        	
        }
        ...
        if(empty($order)){
            ...
            foreach ($order as $row) {
                if($search != ""){
                    if ($row['column'] == 2) {
                        $order_query = $order_query == "" ? " o.payment_status ".$row['dir'] : $order_query.", o.payment_status ".$row['dir'];
                    }else if ($row['column'] == 3) {
                        $order_query = $order_query == "" ? " o.status ".$row['dir'] : $order_query.", o.status ".$row['dir'];
                    }else if ($row['column'] == 4) {
                        $order_query = $order_query == "" ? " o.order_status ".$row['dir'] : $order_query.", o.order_status ".$row['dir'];
                    }else if ($row['column'] == 5) {
                        $order_query = $order_query == "" ? " o.insert_time ".$row['dir'] : $order_query.", o.insert_time ".$row['dir'];
                    }
                }else{
                    if ($row['column'] == 2) {
                        $order_query = $order_query == "" ? " payment_status ".$row['dir'] : $order_query.", payment_status ".$row['dir'];
        ...
        $result['data'] = [];
        if ($search != "") {
            $order_list = $this->Api_Model->get_datatables_list(TBL_ORDER . " o", "o.*, o.id as id, u.fullname, u.username", $where_query, $where_group_like_query, $where_group_or_like_query, $order_query, $start, $count, array(), TBL_USER . " u", $left_join_query, array(), array(), "");
        }else{
        ...
        foreach ($order_list as $row) {
            $order_detail_join_id = ($row['user_id'] == 0) ? $row['tmp_user_id'] : $row['user_id'];
            $order_detail_display_type = ($row['user_id'] == 0) ? "tmp_user_id" : "user_id";
            $user_id = ($row['user_id'] == 0) ? $row['referral_id'] : $row['user_id'];
            $purchase_type = ($row['user_id'] == 0) ? "Referral: " : "Agent: ";
            $agent_info = $this->Api_Model->get_rows_info(TBL_USER, "id, username, fullname, phone_no", array('id' => $user_id, 'active' => 1));
            if(isset($agent_info['id']) && $agent_info['id'] > 0){
                $username = $agent_info['username'];
                $fullname = $agent_info['fullname'];
                $phone_no = $agent_info['phone_no'];
                $type = $purchase_type;
            }else{
                $username = "";
                $fullname = "";
            ...
            $product_order_list = "";
            $product_list = $this->Api_Model->get_rows(TBL_ORDER_DETAIL, "*", array('order_id' => $row['id'], 'active' => 1, $order_detail_display_type => $order_detail_join_id));
            ...
            if(!empty($product_list)){
                ...
                $product_order_list .= "Order ID: #000" . $row['id'] . "<br>" . "(" . $order_t . ")" . "<br><br>";
                $grand_total = $row['total_price'] + $row['delivery_fee'];
                foreach($product_list as $row_product){
                    ...
                    if(isset($product_info['id']) && $product_info['id'] > 0){
                        $product_order_list .= "<span><b>" . $product_info['name'] . " x " . $row_product['quantity'] . "</b><br>";
                        // $product_order_list .= "<img width='80' src='" . DISPLAY_PATH . "img/product/" . $product_info['image'] . "'><br><span>" . $product_info['name'] . " x " . $row_product['quantity'] . "<br>";
                    }else{
                        $product_order_list = "";
                    }
                }
                $product_order_list .= "<br>Total Price: " . number_format($grand_total, 2) . "</span><br>" . "Service Tax: " . $row['extra_charge'] . "<br>" . "Delivery Fee: " . $row['delivery_fee'] . "<br>" . "Subtotal: " . number_format($grand_total, 2) . "<br>";
            }else{
                $row['product_name'] = "";
            }

            $row['product_name'] = $product_order_list;
            ...
            if($payment_status == "UNPAID"){
                ...
                if($row['payment_receipt'] != NULL){
                    ...
                    $row['payment_status'] = '<span class="badge bg-light-success mb-1 mr-2">' . $payment_status . '</span>';
                }
            }
            ...
            if($order_status == "PENDING"){
                ...
                $btn .= "<a href='#' class='btn-sm btn-success' style='border:none;' onclick='approve_order(" . $row['id'] . ")'>" . "Approve" . "</a> ";
                $btn .= "<a href='#' class='btn-sm btn-danger' style='border:none;' onclick='cancel_order(" . $row['id'] . ")'>" . "Cancel" . "</a> ";
                $btn .= "<a href='#' class='btn-sm btn-dark' style='border:none;' onclick='refund_stock_order(" . $row['id'] . ")'>" . "Refund Stock" . "</a> ";
            }else if($order_status == "APPROVE" && $shipment_status == "PLACED"){
                ...
                if($company_id == 0){
                    $btn .= "<a href='#' class='btn-sm btn-danger' style='border:none;' onclick='revert_cancel_order(" . $row['id'] . ")'>" . "Revert Order" . "</a> ";
                }
            }else{
                $row['status'] = '<span class="badge bg-light-danger mb-1 mr-2">' . $order_status . '</span>';
            }
            ...
            if($user_type == "ADMIN"){
                $btn .= "<a href='#' class='btn-sm btn-info' onclick='update_order_status(" . $row['id'] . ")' style='border:none;'>" . "Update" . "</a> ";
            }

            $row['order_ids'] = "Order ID: #000" . $row['id'];
            $row['order_agent'] = $fullname . " (" . $username . ")" . "<br>" . $phone_no;
    ...
    public function get_shipment(){
        ...
        $user_id = isset($this->request_data['user_id']) ? $this->request_data['user_id'] : 0;
        $user_type = isset($this->request_data['user_type']) ? $this->request_data['user_type'] : "";
        $order_status = isset($this->request_data['order_status']) ? $this->request_data['order_status'] : "";
        $status = isset($this->request_data['status']) ? $this->request_data['status'] : "";
        $company_id = isset($this->request_data['company_id']) ? $this->request_data['company_id'] : 0;
        $group_id = isset($this->request_data['group_id']) ? $this->request_data['group_id'] : 0;
        $is_agent = isset($this->request_data['is_agent']) ? $this->request_data['is_agent'] : 0;
        $is_referral = isset($this->request_data['is_referral']) ? $this->request_data['is_referral'] : 0;
        $is_checking = isset($this->request_data['is_checking']) ? $this->request_data['is_checking'] : 0;
        ...
        if($user_type == "ADMIN" && $group_id == 1){
            if($is_agent == 1){
                $where_query = array('active' => 1, 'user_id' => $user_id, 'status' => "APPROVE");
                if($is_checking == 1){
                    $where_query['is_order_checked'] = 0;
                }
            }else if($is_referral == 1){
                $where_query = array('active' => 1, 'referral_id' => $user_id, 'type' => "restock", 'status' => "APPROVE");
                if($is_checking == 1){
                    $where_query['is_order_checked'] = 0;
                }
            }else{
                $where_query = array('active' => 1, 'status' => "APPROVE");
                if($is_checking == 1){
                    $where_query['is_order_checked'] = 0;
                }
            }
            if($status != ""){
                $where_query['status'] = $status;
                $where_query['order_status'] = $order_status;
                if($is_checking == 1){
                    $where_query['is_order_checked'] = 0;
                }
            }
            ...
            if($is_agent == 1){
                $where_query = array('active' => 1, 'company_id' => $company_id, 'user_id' => $user_id, 'status' => "APPROVE");
                if($is_checking == 1){
                    $where_query['is_order_checked'] = 0;
                }
            }else if($is_referral == 1){
                ...
                $where_query = array('active' => 1, 'company_id' => $company_id, 'status' => "APPROVE");
                if($is_checking == 1){
                    $where_query['is_order_checked'] = 0;
                }
            }
        ...
        if ($search != "") {
            ...
            $where_group_or_like_query = array('s_name' => $search, 's_contact' => $search, 's_email' => $search, 'tracking_no' => $search);
        ...
        foreach ($order_list as $row) {
            ...
            if($payment_status == "UNPAID"){
                ...
                if($row['payment_receipt'] != NULL){
                    ...
                    $payment_badges = '<span class="badge bg-light-success mb-1 mr-2">' . $payment_status . '</span>';
                }
            }
            ...
            if(!empty($product_list)){
                ...
                $product_order_list .= "Order ID: #000" . $row['id'] . " (" . $order_t . ")" .  "<br><br>";
                $grand_total = $row['total_price'] + $row['delivery_fee'];
                foreach($product_list as $row_product){
                    ...
                    if(isset($product_info['id']) && $product_info['id'] > 0){
                        ...
                        $product_order_list .= "<span><b>" . $product_info['name'] . " x " . $row_product['quantity'] . "</b><br>";
                        $product_item_list .= "<span><b>" . $product_info['name'] . " x " . $row_product['quantity'] . "</b><br>";
                        // $product_order_list .= "<img width='80' src='" . DISPLAY_PATH . "img/product/" . $product_info['image'] . "'><br><span>" . $product_info['name'] . " x " . $row_product['quantity'] . "<br>";
                    }else{
                        $product_order_list = "";
                    }
                }
                $product_order_list .= "<br>Total Price: " . number_format($grand_total, 2) . "</span><br>" . "Delivery Fee: " . $row['delivery_fee'] . "<br>" . "Subtotal: " . number_format($grand_total, 2) . "<br><br>" . $payment_badges . "<br>";
            }else{
                $row['product_name'] = "";
            }
            $row['product_name'] = $product_order_list;
            $row['product_item'] = rtrim($product_item_list,',');
            ...
            if($order_user_id == 0){
                if($referral_username == "" || $referral_fullname == ""){
                    $row['upline_name'] = $referral_username;
                }else{
                    $row['upline_name'] = $referral_fullname . " (" . $referral_username . ")";
                }
            }else{
                if($username == "" || $fullname == ""){
                    $row['upline_name'] = $fullname;
                }else{
                    $row['upline_name'] = $fullname . " (" . $username . ")";
                }
            }
            ...
            $id = str_replace(array('+', '/', '='), array('-', '_', '~'), $this->encryption->encrypt($row['id']));
            if($user_type == "ADMIN"){
                $btn .= "<a href='" . site_url() . "Order/eorder/" . $id . "' class='btn-sm btn-info' style='border:none;'>" . "Edit" . "</a> <a href='#' class='btn-sm btn-danger' onclick='update_order_status(" . $row['id'] . ")' style='border:none;'>" . "Update" . "</a> <a href='" . site_url() . "Order/generate/" . $row['id'] . "' target='_blank' class='btn-sm btn-success' style='border:none;'>" . "Invoice" . "</a>";
            }

            $row['shipment_fullname'] = $shipping_name;
            $row['shipment_phone_no'] = $shipping_contact;
            $row['shipment_postcode'] = $shipping_postcode;
            $row['shipment_address'] = $shipping_address;
            // if($row['total_quantity'] == 12 && $row['type'] == "normal"){
            //     $shipment_weight = ($row['total_quantity'] - 2) * 0.3;
            // }else{
            //     $shipment_weight = $row['total_quantity'] * 0.3;
            // }
            $shipment_weight = ($total_gram + 600) / 1000;
            $row['shipment_weight'] = $shipment_weight;
            $row['shipping_order_type'] = "document";
            if($company_id == 2){
                $row['shipping_order_item'] = "Sangrila Item";
            }else{
                $row['shipping_order_item'] = "Mudah Makan Item";
            }
            $row['shipping_waybill'] = "";
            $row['shipping_remark'] = "#000" . $row['id'] . " " . $shipping_remark;
            $row['shipping_order_id'] = "#000" . $row['id'];
            $row['shipping_payment'] = $payment_badges;
    ...
    public function checking_order(){
        $order_id_arr = isset($this->request_data['order_id']) ? $this->request_data['order_id'] : [];
    ...
    public function generate_invoice($order_id)
    {
        ...
        $html = $this->load->view('main/package_invoice',$this->page_data,true);
        $mpdf->WriteHTML($html);
    ...
    public function pgenerate($order_id)
    {
        $mpdf = new \Mpdf\Mpdf(['tempDir' => '/tmp', 'mode' => '+aCJK', 'autoScriptToLang' => true, 'autoLangToFont' => true]);
        $this->page_data['package_info'] = $this->Api_Model->get_info_sql(TBL_PURCHASE_PACKAGE, "*", "WHERE id = '$order_id'");
        $html = $this->load->view('main/package_invoice',$this->page_data,true);
    ...
    public function generate_checked_list()
    {
        $mpdf = new \Mpdf\Mpdf(['tempDir' => '/tmp', 'mode' => '+aCJK', 'autoScriptToLang' => true, 'autoLangToFont' => true]);
        $user_id = $this->session->userdata("user_id");
        $user_info = $this->Api_Model->get_rows_info(TBL_USER, "id, company_id", array('id' => $user_id));
        $company_id = isset($user_info['id']) ? $user_info['company_id'] : 0;
        $this->page_data['order_list'] = $this->Api_Model->get_all_sql(TBL_ORDER, "*", "WHERE status = 'APPROVE' AND order_status = 'PLACED' AND active = '1' AND company_id = '$company_id'");
        $this->page_data['company_id'] = $company_id;
        $html = $this->load->view('main/shipment_checking_list',$this->page_data,true);
        $mpdf->WriteHTML($html);
        $mpdf->Output(); // opens in browser
    }
    ...
    public function update_shipment_order(){
        $order_id = isset($this->request_data['order_id']) ? $this->request_data['order_id'] : "";
        $s_name = isset($this->request_data['s_name']) ? $this->request_data['s_name'] : "";
        $s_email = isset($this->request_data['s_email']) ? $this->request_data['s_email'] : "";
        $s_contact = isset($this->request_data['s_contact']) ? $this->request_data['s_contact'] : "";
        $s_address = isset($this->request_data['s_address']) ? $this->request_data['s_address'] : "";
        $s_city = isset($this->request_data['s_city']) ? $this->request_data['s_city'] : "";
        $s_postcode = isset($this->request_data['s_postcode']) ? $this->request_data['s_postcode'] : "";
        $s_state = isset($this->request_data['s_state']) ? $this->request_data['s_state'] : "";
        $s_country = isset($this->request_data['s_country']) ? $this->request_data['s_country'] : "";
        $s_remark = isset($this->request_data['s_remark']) ? $this->request_data['s_remark'] : "";
        ...
        $this->Api_Model->update_data(TBL_ORDER, array('id' => $order_id, 'active' => 1), $data_order);
