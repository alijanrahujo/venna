                    ...
                    <div class="col-12">
                        <div class="card">
                            <div class="card-header">
                                <h4 class="card-title"><?php echo $this->lang->line("category_list"); ?> <a href="<?= site_url(); ?>Category/add"><i class="fa fa-plus-circle add-icon"></i></a></h4>
                            </div>
                            <div class="card-content ">
                                <div class="card-body">
                                    <div class="table-responsive">
                                        <table class="table table-striped table-bordered" id="category_dt">
                                            <thead>
                                                <tr>
                                                    <th>#</th>
                                                    <th>Name</th>
...
<script type="text/javascript">
	...
	function drawtable(){
		$('#category_dt').dataTable().fnClearTable();
    	$('#category_dt').dataTable().fnDestroy();
        $('#category_dt').DataTable({
            'order': [],
            'serverSide': true,
            "bStateSave": true,
            ...
            'ajax': {
                'url': address + "Category/get_category",
                "type": "POST",
                "data": {
    ...
    function delete_category(id){
        if (confirm("Are you sure you want to delete ?")) {
            ...
            delete_category.set('access_token', $("#access_token").val());
            delete_category.set('id', id);

            axios.post(address + 'Category/delete_category' , delete_category, apiHeader)
            .then(function (response) {
                if(response.data.status == "Success"){
                    success_response_with_timer('<?php echo $this->lang->line("delete_success"); ?>', true, 'Category');
                }else{
