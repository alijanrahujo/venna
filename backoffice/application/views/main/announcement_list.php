                    ...
                    <div class="col-12">
                        <div class="card">
                            <div class="card-header">
                                <h4 class="card-title"><?php echo $this->lang->line("announcement_list"); ?> <a href="<?= site_url(); ?>Company/announcementa"><i class="fa fa-plus-circle add-icon"></i></a></h4>
                            </div>
                            <div class="card-content ">
                                <div class="card-body">
                                    <div class="table-responsive">
                                        <table class="table dt-responsive" id="announcement_dt" width="100%">
                                            <thead>
                                                <tr>
                                                    <th>Title</th>
                                                    <th>Date</th>
...
<script type="text/javascript">
	...
	function drawtable(){
		$('#announcement_dt').dataTable().fnClearTable();
    	$('#announcement_dt').dataTable().fnDestroy();
        $('#announcement_dt').DataTable({
            'order': [],
            'serverSide': true,
            "bStateSave": true,
            ...
            'ajax': {
                'url': address + "Company/get_announcement",
                "type": "POST",
                "data": {
    ...
    function delete_announcement(id){
        if (confirm("<?php echo $this->lang->line("are_you_sure_you_want_to_delete"); ?> ?")) {
            var delete_announcement = new FormData();
            delete_announcement.set('access_token', $("#access_token").val());
            delete_announcement.set('id', id);

            axios.post(address + 'Company/delete_announcement' , delete_announcement, apiHeader)
            .then(function (response) {
                if(response.data.status == "Success"){
                    success_response_with_timer('<?php echo $this->lang->line("delete_success"); ?>', true, 'Company/announcement');
                }else{
