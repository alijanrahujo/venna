                    ...
                    <div class="col-12">
                        <div class="card">
                            <div class="card-header">
                                <h4 class="card-title"><?php echo $this->lang->line("company_list"); ?> <a href="<?= site_url(); ?>Company/add"><i class="fa fa-plus-circle add-icon"></i></a></h4>
                            </div>
                            <div class="card-content ">
                                <div class="card-body">
                                    <div class="table-responsive">
                                        <table class="table dt-responsive" id="company_dt" width="100%">
                                            <thead>
                                                <tr>
                                                    <th>#</th>
...
<script type="text/javascript">
	...
	function drawtable(){
		$('#company_dt').dataTable().fnClearTable();
    	$('#company_dt').dataTable().fnDestroy();
        $('#company_dt').DataTable({
            'order': [],
            'serverSide': true,
            "bStateSave": true,
            ...
            'ajax': {
                'url': address + "Company/get_company",
                "type": "POST",
                "data": {
    ...
    function delete_company(id){
        if (confirm("<?php echo $this->lang->line("are_you_sure_you_want_to_delete"); ?> ?")) {
            var delete_company = new FormData();
            delete_company.set('access_token', $("#access_token").val());
            delete_company.set('id', id);

            axios.post(address + 'Company/delete_company' , delete_company, apiHeader)
            .then(function (response) {
                if(response.data.status == "Success"){
                    success_response_with_timer('<?php echo $this->lang->line("delete_success"); ?>', true, 'Company');
                }else{
