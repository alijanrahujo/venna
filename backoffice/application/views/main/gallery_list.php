                    ...
                    <div class="col-12">
                        <div class="card">
                            <div class="card-header">
                                <h4 class="card-title"><?php echo $this->lang->line("gallery_list"); ?> <a href="<?= site_url(); ?>Gallery/add"><i class="fa fa-plus-circle add-icon"></i></a></h4>
                            </div>
                            <div class="card-content ">
                                <div class="card-body">
                                    <div class="table-responsive">
                                        <table class="table dt-responsive" id="gallery_dt" width="100%">
                                            <thead>
                                                <tr>
...
<script type="text/javascript">
	...
	function drawtable(){
		$('#gallery_dt').dataTable().fnClearTable();
    	$('#gallery_dt').dataTable().fnDestroy();
        $('#gallery_dt').DataTable({
            'order': [],
            'serverSide': true,
            "bStateSave": true,
            ...
            'ajax': {
                'url': address + "Gallery/get_gallery",
                "type": "POST",
                "data": {
    ...
    function delete_gallery(id){
        if (confirm("<?php echo $this->lang->line("are_you_sure_you_want_to_delete"); ?> ?")) {
            var delete_gallery = new FormData();
            delete_gallery.set('access_token', $("#access_token").val());
            delete_gallery.set('id', id);

            axios.post(address + 'Gallery/delete_gallery' , delete_gallery, apiHeader)
            .then(function (response) {
                if(response.data.status == "Success"){
                    success_response_with_timer('<?php echo $this->lang->line("delete_success"); ?>', true, 'Gallery');
                }else{
