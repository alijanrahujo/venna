...
<script>
    ...
    $('#announcement_form').submit(function(e) {
        ...
        var insert_announcement = new FormData(this);
        insert_announcement.set('access_token', $("#access_token").val());
        insert_announcement.set('update_by', $("#insert_by").val());

        axios.post(address + 'Company/insert_announcement' , insert_announcement, apiHeader)
        .then(function (response) {
            if(response.data.status == "Success"){
                success_response('Add Successfully !', true, 'Company/announcementa');
            }else{
                warning_response(response.data.message);
            }
        })
