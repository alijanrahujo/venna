<div class="main-panel">
    <div class="main-content">
        <div class="content-overlay"></div>
        <div class="content-wrapper">
            <section id="file-export">
                <div class="row">
                    <div class="col-12">
                        <div class="card">
                            <div class="card-header">
                                <h4 class="card-title"><?php echo $this->lang->line("shipment_report"); ?></h4>
                            </div>
                            <div class="card-content ">
                                <div class="card-body">
                                    <div class="table-responsive">
                                        <table class="table" id="shipment_report_dt">
                                            <thead>
                                                <tr>
                                                    <th>Order ID</th>
                                                    <th>Type</th>
                                                    <th>Agent Name</th>
                                                    <th>Agent Contact</th>
                                                    <th>Shipment Name</th>
                                                    <th>Shipment Contact</th>
                                                    <th>Shipment Address</th>
                                                    <th>Shipment City</th>
                                                    <th>Shipment State</th>
                                                    <th>Shipment Postcode</th>
                                                    <th>Shipment Location</th>
                                                    <th>Shipment Remark</th>
                                                    <th>Product Details</th>
                                                    <th>Payment Status</th>
                                                    <th>Delivery Company</th>
                                                    <th>Tracking No</th>
                                                    <th>Tracking Url</th>
                                                    <th>Datetime</th>
                                                </tr>
                                            </thead>
                                            <tbody></tbody>
                                        </table>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </section>
        </div>
    </div>
</div>

<script type="text/javascript">
	$(document).ready(function(){
        drawtable_pending();
	});

    function drawtable_pending(){
		$('#shipment_report_dt').dataTable().fnClearTable();
    	$('#shipment_report_dt').dataTable().fnDestroy();
        $('#shipment_report_dt').DataTable({
            'dom': 'Bfrtip',
            'buttons': [
                {
                    extend: 'excelHtml5',
                    title: 'Shipment Report',
                    text: 'Export to Excel'
                },
            ],
            'order': [],
            'searching' : false,
            'serverSide': true,
            "bStateSave": true,
            "fnStateSave": function (oSettings, oData) {
                localStorage.setItem('offersDataTables', JSON.stringify(oData));
            },
            "fnStateLoad": function (oSettings) {
                return JSON.parse(localStorage.getItem('offersDataTables'));
            },
            'ajax': {
                'url': address + "Report/get_shipment_report",
                "type": "POST",
                "data": {
                    access_token: $("#access_token").val(),
                    language: '<?php echo $this->session->userdata("site_lang"); ?>',
                    user_id: '<?php echo $this->session->userdata("user_id"); ?>',
                    user_type: '<?php echo $this->session->userdata("user_type"); ?>',
                    status: 'PLACED',
                    group_id: '<?php echo $this->user_profile_info['group_id']; ?>',
                    company_id: '<?php echo $this->user_profile_info['company_id']; ?>'
                },
                dataFilter: function (data) {
                    var result = jQuery.parseJSON(data);
                    if (result.status == "Failed") {
                        Swal.fire({
                            title: '<?php echo $this->lang->line("warning"); ?>',
                            text: result,
                            confirmButtonText: '<?php echo $this->lang->line("ok"); ?>'
                        })
                        return JSON.stringify(result.data);
                    }else{
                        return JSON.stringify(result.data);
                    }
                }
            },
            "language": {
                "lengthMenu": "<?php echo $this->lang->line("showing"); ?>" + " _MENU_ " + "<?php echo $this->lang->line("entries"); ?>",
                "info": "<?php echo $this->lang->line("showing"); ?>" + " _PAGE_ / _PAGES_ " + "<?php echo $this->lang->line("pages"); ?>",
                "infoEmpty": "<?php echo $this->lang->line("record_not_found"); ?>",
                "zeroRecords": "<?php echo $this->lang->line("record_not_found"); ?>",
                "infoFiltered": "(" + "<?php echo $this->lang->line("filter_form"); ?>" + " _MAX_ " + "<?php echo $this->lang->line("total_records"); ?>" + ")",
                "search": "<?php echo $this->lang->line("filter_search"); ?>" + ":",
                "paginate": {
                "previous": "<?php echo $this->lang->line("previous"); ?>",
                "next": "<?php echo $this->lang->line("next"); ?>",
                }
            },
            "columns": [
                {"data": "order_id", "orderable": false},
                {"data": "order_type", "orderable": false},
                {"data": "agent_name", "orderable": false},
                {"data": "agent_contact", "orderable": false},
                {"data": "shipment_name", "orderable": false},
                {"data": "shipping_contact", "orderable": false},
                {"data": "shipping_address", "orderable": false},
                {"data": "shipping_city", "orderable": false},
                {"data": "shipping_state", "orderable": false},
                {"data": "shipping_postcode", "orderable": false},
                {"data": "shipping_country", "orderable": false},
                {"data": "shipping_remark", "orderable": false},
                {"data": "product_details", "orderable": false},
                {"data": "payment_status", "orderable": false},
                {"data": "delivery_company", "orderable": false},
                {"data": "tracking_no", "orderable": false},
                {"data": "tracking_url", "orderable": false},
                {"data": "insert_time", "orderable": false},
            ]
        });
        $('.dt-buttons').addClass('btn-group');
        $('.buttons-excel').addClass('btn btn-primary mb-2');
    }
</script>