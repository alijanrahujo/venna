                    ...
                    <div class="col-12">
                        <div class="card">
                            <div class="card-header">
                                <h4 class="card-title"><?php echo $this->lang->line("voucher_list"); ?> <a href="<?= site_url(); ?>Voucher/open"><i class="fa fa-plus-circle add-icon"></i></a></h4>
                            </div>
                                        ...
                                        <table class="table dt-responsive" id="voucher_dt" width="100%">
                                            <thead>
                                                <tr>
                                                    ...
                                                    <th><?php echo $this->lang->line("package"); ?> <?php echo $this->lang->line("package"); ?></th>
    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h4 class="modal-title" id="myModalLabel1">Update Voucher Used By Agent</h4>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true"><i class="ft-x font-medium-2 text-bold-700"></i></span>
                </button>
            </div>
            <div class="modal-body">
                <form method="POST" id="used_voucher_form">
                    <div class="row">
                        <div class="col-md-12">
                            <fieldset class="form-group">
                                <label for="basicInput">Agent</label>
                                <select class="form-control" name="register_user_id">
                                    <?php
...
<script type="text/javascript">
        ...
        if (confirm("<?php echo $this->lang->line("are_you_sure_you_want_to_delete"); ?> ?")) {
            ...
            .then(function (response) {
                if(response.data.status == "Success"){
                    success_response_with_timer('<?php echo $this->lang->line("delete_success"); ?>', true, 'Voucher/package');
                }else{
    ...
    function show_voucher_update(id){
        $("#voucher_id").val(id);
        $("#voucherModal").modal("show");
    }

    $('#used_voucher_form').submit(function(e) {
        e.preventDefault();

        var update_voucher = new FormData(this);
        update_voucher.set('access_token', $("#access_token").val());
        update_voucher.set('insert_by', $("#insert_by").val());

        axios.post(address + 'Voucher/update_voucher_log' , update_voucher, apiHeader)
        .then(function (response) {
            if(response.data.status == "Success"){
                success_response('Update Success !', true, 'Voucher/package');
            }else{
                warning_response(response.data.message);
            }
        })
